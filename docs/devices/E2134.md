...
- [FCC](https://apps.fcc.gov/oetcf/eas/reports/ViewExhibitReport.cfm?mode=Exhibits&RequestTimeout=500&calledFromFrame=N&application_id=EuRz2ghtxr%2BwKMWkZRrjGg%3D%3D&fcc_id=FHO-E2134) - emissisons, teardown, manual
- [CSA IoT](https://csa-iot.org/csa_product/vallhorn-motion-sensor/) - Zigbee compliance

### Battery
- Uses 2 x AAA **rechargeable** battery (1.2V)
- Alkaline batteries (1.5V) will cause unexpected behavior, such as false-positives
- See Mods to lower Alkaline voltage 

This device *sleeps* to preserve energy when there is nothing to report.  
When sleeping, it will not respond to any Z2M commands.  
**Trigger its sensors to keep it awake and allow interactions**, 
for example when attempting to bind.

### Pairing
**Press the pair button 4 times** to factory reset and start pairing.   
The front side should flash and pulsate red until it connects.  
...
- Subscribed to *manufacturerCode* `4476`, *imageType* `6456`
